...
function! s:repeat_set(buf, count) abort
...
function s:undo_ftplugin_add(command) abort
...
function s:create_plugs() abort
  ...
  inoremap <buffer><silent><expr> <Plug>(sexp_insert_opening_round)  sexp#opening_insertion('(')
  inoremap <buffer><silent><expr> <Plug>(sexp_insert_opening_square) sexp#opening_insertion('[')
  inoremap <buffer><silent><expr> <Plug>(sexp_insert_opening_curly)  sexp#opening_insertion('{')

  " Insert closing delimiter
  inoremap <buffer><silent><expr> <Plug>(sexp_insert_closing_round)  sexp#closing_insertion(')')
  inoremap <buffer><silent><expr> <Plug>(sexp_insert_closing_square) sexp#closing_insertion(']')
  inoremap <buffer><silent><expr> <Plug>(sexp_insert_closing_curly)  sexp#closing_insertion('}')

  " Insert double quote
  inoremap <buffer><silent><expr> <Plug>(sexp_insert_double_quote) sexp#quote_insertion('"')

  " Delete paired delimiters
  inoremap <buffer><silent><expr> <Plug>(sexp_insert_backspace) sexp#backspace_insertion()
...
" s:sexp_mappings
function! s:sexp_create_mappings() abort
  ...
  for plug in ['sexp_outer_list',     'sexp_inner_list',
    ...
    if !empty(lhs)
      ...
      call s:undo_ftplugin_add('execute "silent! xunmap <buffer> '.lhs.'"')
      execute 'omap <silent><buffer> ' . lhs . ' <Plug>(' . plug . ')'
      call s:undo_ftplugin_add('execute "silent! ounmap <buffer> '.lhs.'"')
    endif
  endfor
  ...
  for plug in ['sexp_move_to_prev_bracket',      'sexp_move_to_next_bracket',
    ...
    if !empty(lhs)
      ...
      call s:undo_ftplugin_add('execute "silent! nunmap <buffer> '.lhs.'"')
      execute 'xmap <silent><buffer> ' . lhs . ' <Plug>(' . plug . ')'
      call s:undo_ftplugin_add('execute "silent! xunmap <buffer> '.lhs.'"')
      execute 'omap <silent><buffer> ' . lhs . ' <Plug>(' . plug . ')'
      call s:undo_ftplugin_add('execute "silent! ounmap <buffer> '.lhs.'"')
